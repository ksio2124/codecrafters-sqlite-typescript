...
import { IndexInteriorCell, IndexLeafCell } from "./IndexCell"


export class IndexInteriorPage {
  ...
  getCells() {
    return this.getCellPtrs().map((cellPtr) => new IndexInteriorCell(cellPtr, this.pageBuffer))
  }
...
export class IndexLeafPage {
  ...
  getCells() {
    return this.getCellPtrs().map((cellPtr) => new IndexLeafCell(cellPtr, this.pageBuffer))
  }
}